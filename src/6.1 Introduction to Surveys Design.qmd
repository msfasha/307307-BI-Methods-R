...
Non-probability sampling methods, such as **convenience sampling**, **quota sampling**, and **snowball sampling**, are commonly used when random sampling is impractical or unnecessary. These methods can introduce bias, so results should be interpreted cautiously. Below are examples for **convenience sampling** and **snowball sampling**.

### **1. Convenience Sampling**

#### Definition:

A sampling technique where participants are selected based on their **availability** or **ease of access**.

#### Example:

-   **Scenario**: A researcher wants to study the coffee-drinking habits of university students.

-   **Method**:

    -   The researcher stands near the university library and surveys students passing by.

    -   Participants are chosen because they are conveniently located and easy to approach.

-   **Limitations**:

    -   The sample may not represent the entire student population (e.g., students who don’t frequent the library might have different coffee-drinking habits).

    -   May lead to biases like over-representing certain groups.

### **2. Snowball Sampling**

#### Definition:

A technique used to recruit participants through **referrals** from initial subjects, often used in hard-to-reach populations.

#### Example:

-   **Scenario**: A sociologist wants to study the experiences of undocumented workers in a city.

-   **Method**:

    -   The researcher contacts one undocumented worker and conducts an interview.

    -   This participant refers the researcher to others they know, who then refer additional participants.

-   **Limitations**:

    -   The sample is unlikely to be random, as participants are chosen through social networks.

    -   Results may be skewed toward individuals within specific subgroups or social circles.

### **3. Key Differences**

| **Sampling Method** | **Example Context** | **Strengths** | **Weaknesses** |
|----|----|----|----|
| **Convenience Sampling** | Survey students at a university library | Quick and easy to collect data | Limited generalizability; high bias |
| **Snowball Sampling** | Research on undocumented workers | Useful for reaching hidden populations | Dependent on referrals; less diverse |

### **Caution in Non-Probability Sampling**

Non-random selection in these methods introduces potential **bias**:

-   Results might not generalize to the broader population.

-   Certain groups may be over- or under-represented.

Despite these limitations, these methods are valuable when random sampling is impractical or when studying specialized or hard-to-reach populations.
...
When estimating a population mean, the sample size formula is: \[ n = \frac{{Z^2 \cdot \sigma^2}}{{E^2}} \] Where: - (n): Required sample size. - (Z): Z-score corresponding to the desired confidence level. - (\sigma): Standard deviation of the population (if unknown, estimate it from a pilot study or prior data). - (E): Desired margin of error (maximum acceptable difference between the sample mean and true population mean).
...
1.  **Determine Parameters**:
    -   Confidence level: E.g., 95% ((Z = 1.96)).
    -   Margin of error ((E)): Decide the acceptable range for the mean estimate (e.g., ±500 JDs).
    -   Population standard deviation ((\sigma)): Estimate from prior studies or conduct a small pilot survey.
2.  **Apply the Formula**: Substitute (Z), (\sigma), and (E) into the formula to calculate (n).

------------------------------------------------------------------------

### **Example**

Suppose you want to estimate the **average income** of 10,000 students: - (Z = 1.96) (95% confidence level). - (\sigma = 5,000) JDs (estimated standard deviation from prior data). - (E = 500) JDs (desired margin of error).

$$  n = \frac{{1.96^2 \cdot 5000^2}}{{500^2}} $$

1.  Calculate the numerator: $$  1.96\^2 \cdot 5000\^2 = 3.8416 \cdot 25,000,000 = 96,040,000 $$

2.  Calculate the denominator: \[ 500\^2 = 250,000 \$\$

3.  Compute (n): $$  n = \frac{{96,040,000}}{{250,000}} = 384.16 $$

Thus, the sample size needed is approximately **384**.
...
Age Gender Current year of study

Home distance from University
...
If the student work, then ask the following quesations
