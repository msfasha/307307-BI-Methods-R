...
final_df <- data.frame(
  Player = rep(Players, each = length(Seasons)),
  Season = rep(Seasons, times = length(Players)),
  Salary = as.vector(t(Salary)),
  Games = as.vector(t(Games)),
  MinutesPlayed = as.vector(t(MinutesPlayed))
)

# View the Data Frame
print(head(final_df)) # View the first few rows
```
